...
command_t cs_mass_dehalfop = { "DEHALFOP", N_("Makes all channel users on a channel dehalfops."),
...
static void cs_cmd_mass_dehalfop(sourceinfo_t *si, int parc, char *parv[])
{
    ...
    command_success_nodata(si, _("Dehalf-opped all channel users in \2%s\2."), channel);
}

...
command_t cs_mass_owner = { "OWNER", N_("Makes all channel users on a channel owner."),
                            AC_NONE, 2, cs_cmd_mass_owner, { .path = "cservice/mass_owner" }
                          };

command_t cs_mass_deowner = { "DEOWNER", N_("Makes all channel users on a channel deowners."),
...
static void cs_cmd_mass_owner(sourceinfo_t *si, int parc, char *parv[])
{
    ...
    command_success_nodata(si, _("Mass Ownered all users in \2%s\2."), channel);
}
...
static void cs_cmd_mass_deowner(sourceinfo_t *si, int parc, char *parv[])
{
    ...
    logcommand(si, CMDLOG_DO, "MASS:DEOWNERED: \2%s\2", mc->name);

    command_success_nodata(si, _("Mass DeOwnered all users in \2%s\2."), channel);
}
